import sys, pygame
from pygame.locals import * # для использования переменных без приставки pygame(K_l and etc.)



'''
draw_item класс, который создает объект для разных слое, к примеру здесь есть 3 слоя, это work, paint и font.
work - хранить в себе временные  "рисунки", к примеру, границы разных фигур, которые появляються при зажатий ЛКМ
paint - хранить постоянные рисунки
font - описание состояний мыши, и других элементов(верхний правый угол)
'''
class draw_item:
...
class paint:
    ...
    def crosshair(self): # создан чтобы видить границы ластика(Free Eraser)
    ...
    def draw_circle_template(self):
        ...
        и при отпусканий сохраняет коорды конечной точке,
        после этого рисует круг

        !!!! Следующий функций в которых в названиях присутсвует слово template, работают так же как и эта функция
        '''
        if self.draw_toggle == False and self.mousedown and self.mousebutton == 1:
            ...
            try: pygame.draw.circle(self.work_canvas,(self.GREY),
    ...
    def draw_line_template(self):
        ...
        if self.draw_toggle == False and self.mousedown and self.mousebutton == 1:
            ...
            pygame.draw.line(self.paint_canvas, self.color,
                             (self.drawstart_x, self.drawstart_y),
                             (self.drawend_x, self.drawend_y), 2)
